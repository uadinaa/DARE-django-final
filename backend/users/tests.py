...
class UserAuthTests(APITestCase):
    ...
    def test_user_registration_success(self):
        ...
        self.assertEqual(User.objects.get().profile.role, Profile.Role.USER) # Проверяем роль
    ...
    def test_trainer_registration_success(self):
        ...
        self.assertTrue(User.objects.get().profile.is_trainer) # Проверяем роль через property
    ...
    def test_registration_passwords_mismatch(self):
        ...
        """
        url = reverse('register')
        data = {"username": "testuser", "email": "test@example.com", "password": "password123", "password2": "wrongpassword", "role": Profile.Role.USER}
        response = self.client.post(url, data, format='json')
    ...
    def test_registration_duplicate_username(self):
        ...
        url = reverse('register')
        data = {"username": "existinguser", "email": "test@example.com", "password": "password123", "password2": "password123", "role": Profile.Role.USER}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('username', response.data) # Проверяем, что ошибка связана с username
...
class ProfileViewTests(APITestCase):
    ...
    def test_update_own_profile(self):
        ...
        data = {"bio": new_bio}
        response = self.client.patch(url, data, format='json') # Используем PATCH для частичного обновления
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_cannot_update_other_profile(self):
        ...
        self.assertEqual(self.user2.profile.bio, "Trying to update user1 bio") # ...обновился профиль user2
        self.assertNotEqual(self.user1.profile.bio, "Trying to update user1 bio") # Профиль user1 не изменился
