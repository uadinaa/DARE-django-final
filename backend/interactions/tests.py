...
class InteractionTests(APITestCase):
    def setUp(self):
        ...
        self.trainer1 = User.objects.create_user(username='trainer1', password='password123')
        self.trainer1.profile.role = Profile.Role.TRAINER
        self.trainer1.profile.save()

        self.post_by_trainer = Post.objects.create(author=self.trainer1, content="Trainer's post")
    ...
    def test_list_comments_for_post(self):
        ...
        """
        Comment.objects.create(post=self.post_by_trainer, author=self.user1, content="Comment 1")
        Comment.objects.create(post=self.post_by_trainer, author=self.user2, content="Comment 2")
        url = reverse('post-comments-list', kwargs={'post_pk': self.post_by_trainer.pk})
        response = self.client.get(url) # Не аутентифицируемся
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        # Проверяем наличие комментов (учитывая пагинацию)
        self.assertTrue(len(response.data.get('results', [])) >= 2 or len(response.data) >= 2)

    def test_delete_own_comment(self):
         """ Ensure author can delete their own comment. """
         comment = Comment.objects.create(post=self.post_by_trainer, author=self.user1, content="Comment to delete")
         # URL для конкретного коммента: /api/posts/{post_pk}/comments/{comment_pk}/
         url = reverse('post-comments-detail', kwargs={'post_pk': self.post_by_trainer.pk, 'pk': comment.pk})
         self.client.force_authenticate(user=self.user1)
    ...
    def test_delete_other_comment_forbidden(self):
         ...
         comment = Comment.objects.create(post=self.post_by_trainer, author=self.user1, content="Other's comment")
         url = reverse('post-comments-detail', kwargs={'post_pk': self.post_by_trainer.pk, 'pk': comment.pk})
         self.client.force_authenticate(user=self.user2) # user2 пытается удалить коммент user1
         response = self.client.delete(url)
    ...
    def test_like_post(self):
        ...
        self.assertTrue(PostLike.objects.filter(user=self.user1, post=self.post_by_trainer).exists())

    def test_like_post_already_liked(self):
        """
        Ensure liking an already liked post returns 200 OK.
        """
        PostLike.objects.create(user=self.user1, post=self.post_by_trainer) # Предустановка лайка
        url = reverse('post-like', kwargs={'post_pk': self.post_by_trainer.pk})
        self.client.force_authenticate(user=self.user1)
    ...
    def test_follow_trainer(self):
        ...
        response = self.client.post(url)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(Follow.objects.filter(follower=self.user1, followed=self.trainer1).exists())
    ...
    def test_follow_non_trainer_forbidden(self):
        ...
        """
        url = reverse('follow-user', kwargs={'user_pk': self.user2.pk}) # Пытаемся подписаться на user2
        self.client.force_authenticate(user=self.user1)
        response = self.client.post(url)
    ...
    def test_follow_self_forbidden(self):
        ...
        self.client.force_authenticate(user=self.user1)
        response = self.client.post(url)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST) # Ожидаем ошибку валидации

    def test_unfollow_trainer(self):
        """
        Ensure user can unfollow a trainer.
        """
        Follow.objects.create(follower=self.user1, followed=self.trainer1) # Предустановка подписки
        # URL: /api/interactions/unfollow/{user_pk}/
        url = reverse('unfollow-user', kwargs={'user_pk': self.trainer1.pk})
        self.client.force_authenticate(user=self.user1)
        response = self.client.delete(url)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertFalse(Follow.objects.filter(follower=self.user1, followed=self.trainer1).exists())
    ...
    def test_list_following(self):
         ...
         results = response.data.get('results', response.data) # Учитываем пагинацию или ее отсутствие
    ...
    def test_list_followers(self):
         ...
         self.client.force_authenticate(user=self.user1) # user1 смотрит подписчиков trainer1
         response = self.client.get(url)
         self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_list_followers_of_non_trainer(self):
         ...
         url = reverse('user-follower-list', kwargs={'user_pk': self.user1.pk}) # Смотрим подписчиков user1
