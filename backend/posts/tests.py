...
class PostViewSetTests(APITestCase):
    def setUp(self):
        ...
        self.trainer = User.objects.create_user(username='trainer1', password='password123')
        ...
        self.post1 = Post.objects.create(author=self.trainer, content="First post content")
        self.post2 = Post.objects.create(author=self.trainer, content="Second post content")
    ...
    def test_create_post_as_trainer(self):
        ...
        self.assertEqual(Post.objects.latest('created_at').content, "New post from trainer")
    ...
    def test_create_post_as_regular_user_forbidden(self):
        ...
        """
        url = reverse('post-list')
        self.client.force_authenticate(user=self.user) # Аутентифицируем обычного пользователя
        data = {"content": "Post attempt by regular user"}
        response = self.client.post(url, data, format='json')
        # Ожидаем 403 Forbidden, так как IsTrainer пермишн не пройдет
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(Post.objects.count(), 2) # Убедимся, что новый пост не создался
    ...
    def test_update_other_post_forbidden(self):
        ...
        other_trainer = User.objects.create_user(username='trainer2', password='password123')
        other_trainer.profile.role = Profile.Role.TRAINER
        other_trainer.profile.save()
        other_post = Post.objects.create(author=other_trainer, content="Other trainer's post")

        url = reverse('post-detail', kwargs={'pk': other_post.pk})
        self.client.force_authenticate(user=self.trainer) # Первый тренер пытается обновить пост второго
